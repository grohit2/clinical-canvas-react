...
export type MrnHistoryEntry = { uid: string; scheme: string; date?: string };
type MrnSaveEntry = { uid: string; scheme: string; date: string };
...
export function MrnEditor({
  ...
  const [entries, setEntries] = useState<MrnHistoryEntry[]>(() =>
    (initialHistory && initialHistory.length ? initialHistory : [])
      .map((e) => ({ uid: e.uid || "", scheme: normalizeScheme(e.scheme), date: e.date }))
  );
  ...
  useEffect(() => {
    setEntries((initialHistory || []).map((e) => ({ uid: e.uid || "", scheme: normalizeScheme(e.scheme), date: e.date })));
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (cancelled) return;
        setEntries((p.mrnHistory || []).map((e) => ({ uid: e.uid || "", scheme: normalizeScheme(e.scheme), date: e.date })));
  ...
  const addEntry = () => setEntries((prev) => [...prev, { uid: "", scheme: "", date: "" }]);
  ...
  const setAsLatest = (idx: number) => setLatestMrn(entries[idx]?.uid || "");

  const hasValid = entries.some((e) => e.uid && e.scheme);
  const cleanedHistory = useMemo<MrnSaveEntry[]>(() => {
    ...
    return entries
      ...
      .map((e) => ({
        ...
        date: e.uid === latestMrn ? nowIso : (e.date || oldFallback),
      }));
  }, [entries, latestMrn]);
  ...
  const onSave = async () => {
    ...
    try {
      ...
      try {
        await api.patients.overwriteMrn(patientId, cleanedHistory);
      } catch (e: unknown) {
        ...
        if (!desired || !cleanedHistory.some((h) => h.uid === desired)) {
          desired = [...cleanedHistory].sort((a, b) => new Date(b.date || "1970-01-01").getTime() - new Date(a.date || "1970-01-01").getTime())[0]?.uid || desired;
        }
        if (desired && desired !== originalLatest) {
          const scheme = cleanedHistory.find((h) => h.uid === desired)?.scheme || normalizeScheme();
          await api.patients.switchRegistration(patientId, { uid: desired, scheme });
        }
        await api.patients.updateMrnHistory(patientId, cleanedHistory);
      }
      ...
      toast({ title: "Error", description: getErrorMessage(err) || "Failed to save MRN changes", variant: "destructive" });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-3 sm:p-4">
      ...
      <div className="space-y-3">
        ...
        {entries.map((entry, idx) => (
          <div key={`${idx}-${entry.uid}`} className="border rounded-md p-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`text-xs px-2 py-1 rounded ${
                    latestMrn === entry.uid ? "bg-emerald-600 text-white" : "bg-gray-200 text-gray-800 hover:bg-emerald-100"
                  }`}
                >
                  {latestMrn === entry.uid ? "Latest" : "Set Latest"}
                </button>
                {entries.length > 1 && (
                  <button
                    type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  value={entry.uid}
                  onChange={(e) => updateEntry(idx, "uid", e.target.value)}
