...
export function AddMrnPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!id || !uid.trim() || !scheme) return;
    setSubmitting(true);
    try {
      const payload = { uid: uid.trim(), scheme: normalizeScheme(scheme) };
      await api.patients.switchRegistration(id, payload);
      toast({ title: "MRN added", description: `${uid.trim()} added to ${patientName || "patient"}` });
        ...
        <Card className="p-4 max-w-xl mx-auto">
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              <Button type="submit" disabled={submitting || !uid.trim() || !scheme} className="flex-1">
