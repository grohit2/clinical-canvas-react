...
export const patientService = {
  ...
  async getPatientById(patientId: string): Promise<PatientMeta | null> {
    ...
    return fetchWithFallback(
      () => apiService.get<PatientMeta>(API_CONFIG.PATIENTS.DETAIL, { id: patientId }),
      mockPatient,
      FEATURE_FLAGS.ENABLE_PATIENTS_API
    );
  },
  ...
  async getPatientQRData(patientId: string): Promise<any> {
    const mockData = mockPatientData[patientId as keyof typeof mockPatientData] || null;
  ...
  async createPatient(patientData: Omit<PatientMeta, 'id' | 'qrCode'>): Promise<PatientMeta> {
  ...
  async updatePatient(patientId: string, updates: Partial<PatientMeta>): Promise<PatientMeta> {
    ...
    return fetchWithFallback(
      () => apiService.put<PatientMeta>(API_CONFIG.PATIENTS.UPDATE, updates, { id: patientId }),
      mockUpdatedPatient,
      FEATURE_FLAGS.ENABLE_PATIENTS_API
    );
  },
