...
export const api = {
  patients: {
    ...
    create: (
      ...
      const schemeForRegistration = (data.mrnHistory || []).find(h => h.uid === chosenMrn)?.scheme 
    ...
    update: (uid: string, data: PatientUpdatePayload) => {
      ...
      if (tidStatus !== undefined) shadow['tid_status'] = tidStatus;
      if (tidNumber !== undefined) shadow['tid_number'] = tidNumber;
      if (surgeryCode !== undefined) shadow['surgery_code'] = surgeryCode;
      if (surgeryDate !== undefined) shadow['surgery_date'] = surgeryDate;
      if (roomNumber !== undefined) shadow['room_number'] = roomNumber;
      if (scheme !== undefined) shadow['scheme'] = scheme;
      if (procedureName !== undefined) shadow['procedure_name'] = procedureName;
    ...
    switchRegistration: (
      uid: string,
      data: { uid: string; scheme: string; department?: string; pathway?: string; diagnosis?: string; comorbidities?: string[]; assigned_doctor?: string; assigned_doctor_id?: string; files_url?: string; is_urgent?: boolean; urgent_reason?: string; urgent_until?: string; firstState?: string; actorId?: string }
    ) => {
    ...
    updateMrnHistory: (uid: string, mrnHistory: { uid: string; scheme: string; date: string }[]) =>
      request<{ message: string; patient: Patient }>(`/patients/${uid}/uid-history`, {
        method: 'PATCH',
        body: JSON.stringify({ mrnHistory }),
      }),
    // One-shot: overwrite history and set latest to highest-date in one call
    overwriteMrn: (uid: string, mrnHistory: { uid: string; scheme: string; date: string }[], actorId?: string) =>
      request<{ message: string; patient: Patient }>(`/patients/${uid}/uid-overwrite`, {
        method: 'PATCH',
