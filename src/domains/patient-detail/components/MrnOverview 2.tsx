...
export function MrnOverview({ patientId, mrnHistory, latestMrn, onMrnUpdate }: MrnOverviewProps) {
  ...
  const handleLabClick = (mrnEntry: MrnHistoryEntry) => {
    ...
    const lisUrl = `http://115.241.194.20/LIS/Reports/Patient_Report.aspx?prno=${mrnEntry.uid}`;
  ...
  const handleAddMrn = async () => {
    if (!newMrnData.uid.trim() || !newMrnData.scheme) {
      toast({
    ...
    try {
      ...
      if (onMrnUpdate) onMrnUpdate(updated.mrnHistory || [], updated.latestMrn || payload.uid);

      toast({
        title: "MRN Added Successfully",
        description: `New MRN ${newMrnData.uid} has been added to the patient record`,
      });
  ...
  const currentMrn = sortedMrnHistory.find(entry => entry.uid === latestMrn) || sortedMrnHistory[0];
        ...
        <div className="flex items-center gap-3">
          {!isExpanded && (
            <div className="text-right">
              ...
              <p className="text-xs text-gray-500 mt-1">{currentMrn.uid}</p>
            </div>
          )}
          <Button
            size="sm"
        ...
        <div className="px-4 pb-4 space-y-3 animate-in slide-in-from-top-1 duration-300">
          ...
          <div className="space-y-2">
            {sortedMrnHistory.map((entry, index) => {
              const isCurrent = entry.uid === latestMrn;
              return (
                <div
                  key={entry.uid + index}
      ...
      <Dialog open={showAddMrnDialog} onOpenChange={setShowAddMrnDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="uid">MRN Number *</Label>
              <Input
                id="uid"
                value={newMrnData.uid}
                onChange={(e) => setNewMrnData(prev => ({ ...prev, uid: e.target.value }))}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isSubmitting || !newMrnData.uid.trim() || !newMrnData.scheme}
            >
