...
export const enrichPatient = (patient: Patient): Patient => {
  ...
  const schemeCandidates: Array<string | undefined> = [
    patient.scheme,
    normalizedHistory?.find((entry) => entry.uid === patient.latestMrn)?.scheme,
  ...
  const procedureName = patient.procedureName ?? (patient as Patient & { procedure_name?: string }).procedure_name ?? undefined;
