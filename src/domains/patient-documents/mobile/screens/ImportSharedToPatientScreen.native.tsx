...
export interface ImportSharedPatient {
  id: string;
  name: string;
}
...
export function ImportSharedToPatientScreen({
  ...
  availablePatients: ImportSharedPatient[];
  isLoading: boolean;
  sharedFiles: SharedIntentFile[];
  ...
  const handleImport = async () => {
    ...
    try {
      for (const file of sharedFiles) {
        ...
        const fileName = file.fileName || inferFileName(sourcePath);
            ...
            renderItem={({ item }) => (
              <View style={styles.fileRow}>
                <Text style={styles.fileName} numberOfLines={1}>
                  {item.fileName || inferFileName(item.path)}
                </Text>
