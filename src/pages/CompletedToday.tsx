...
export default function CompletedToday() {
  ...
  const doctorCompletedTasks = mockCompletedTasks.filter(
    (task) => task.completedBy === currentDoctor,
  );
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case "assessment":
        return "bg-medical/5 text-medical border-medical";
      case "medication":
        return "bg-caution/5 text-caution border-caution";
      case "procedure":
        return "bg-stable/5 text-stable border-stable";
      case "lab":
        return "bg-blue-500/5 text-blue-600 border-blue-200";
      default:
      ...
      <div className="p-4 space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {doctorCompletedTasks.length} tasks completed on{" "}
        ...
        <div className="space-y-3">
          {doctorCompletedTasks.map((task) => (
            <Card
              key={task.id}
              className={`p-4 border-l-4 ${getTypeColor(task.type)}`}
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      variant="secondary"
                      className="text-xs bg-stable/10 text-stable"
                    >
                      Completed
                    </Badge>
                  </div>
        ...
        {doctorCompletedTasks.length === 0 && (
          <Card className="p-8 text-center">
            ...
            <p className="text-muted-foreground">
              Tasks you complete today will appear here.
            </p>
          </Card>
        )}
      </div>
