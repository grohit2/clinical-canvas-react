...
export const normalizePatient = (raw: Patient): Patient => {
  ...
  const schemeCandidates: Array<string | undefined> = [
    raw.scheme,
    normalizedHistory?.find((entry) => entry.uid === raw.latestMrn)?.scheme,
  ...
  return {
    ...
    procedureName: raw.procedureName ?? (raw as Patient & { procedure_name?: string }).procedure_name ?? undefined,
  };
};
